...
html/package to create the html used by the web browser, which points to localhost:8080/acyclicsp.
Acyclic shortest paths finds the shortest paths (SP) between a source vertex and all other vertices.
...
const (
	...
	fileAcyclicSP       = "templates/acyclicsp.html"    // html for acyclic SP
	fileGraphOptions    = "templates/graphoptions.html" // html for Graph Options
	patternAcyclicSP    = "/acyclicsp"                  // http handler for acyclic SP connections
...
type PlotT struct {
	...
	SourceLocation string   // source vertex for acyclic SP in x,y coordinates
	TargetLocation string   // target or destination vertex for acyclic SP in x,y coordinates
	Source         string   // source vertex for acyclic SP 0-Vertices-1
	Target         string   // target vertex for acyclic SP 0-Vertices-1
...
// AcyclicSP type for Shortest Path methods
type AcyclicSP struct {
	...
	*Endpoints              // Euclidean graph endpoints
	stack      []int        // Topological sort of vertices
}
...
func init() {
	tmplForm = template.Must(template.ParseFiles(fileAcyclicSP))
}
...
func (asp *AcyclicSP) findSP(r *http.Request) error {
	...
	asp.source, err = strconv.Atoi(sourceVert)
	if err != nil {
		fmt.Printf("source vertex Atoi error: %v\n", err)
		return err
	}
	asp.target, err = strconv.Atoi(targetVert)
	if err != nil {
		fmt.Printf("target vertex Atoi error: %v\n", err)
		return err
	}

	vertices := len(asp.location)
	if asp.source == asp.target || asp.source < 0 || asp.target < 0 ||
		asp.source > vertices-1 || asp.target > vertices-1 {
		return fmt.Errorf("source and/or target vertices are invalid")
	}

	asp.edgeTo = make([]*Edge, vertices)
	asp.distTo = make([]float64, vertices)
	for i := range asp.distTo {
		asp.distTo[i] = math.MaxFloat64
	}
	// Starting index is source, distance to itself is 0
	asp.distTo[asp.source] = 0.0

	// Create the adjacency list
	asp.adj = make([][]*Edge, vertices)
	for i := range asp.adj {
		asp.adj[i] = make([]*Edge, 0)
	}
	for _, e := range asp.mst[1:] {
		asp.adj[e.v] = append(asp.adj[e.v], e)
		asp.adj[e.w] = append(asp.adj[e.w], e)
	}

	// relax the vertex
	relax := func(v int) {
		// find shortest distance from source to w
		for _, e := range asp.adj[v] {
			...
			newDistance := asp.distTo[v] + asp.graph[v][w]
	...
	dfs = func(v int) {
		...
		// Topo sort uses LIFO order for the vertices
		// Vertices will be read in reverse order
		asp.stack[n] = v
		n++
	}
...
func (asp *AcyclicSP) plotSP() error {
	// check if the target was found in findSP
	if len(asp.distTo) == 0 || asp.distTo[asp.target] == math.MaxFloat64 {
		return fmt.Errorf("distance to vertex %d not found", asp.target)
	}
	...
	// Calculate scale factors for x and y
	xscale := (columns - 1) / (asp.xmax - asp.xmin)
	yscale := (rows - 1) / (asp.ymax - asp.ymin)

	beginEP := complex(asp.xmin, asp.ymin) // beginning of the Euclidean graph
	endEP := complex(asp.xmax, asp.ymax)   // end of the Euclidean graph
	lenEP := cmplx.Abs(endEP - beginEP)    // length of the Euclidean graph

	e := asp.edgeTo[asp.target]
	// start at the target and loop until source vertex is plotted to the grid
	for {
		v := e.v
		w := e.w
		start := asp.location[v]
		end := asp.location[w]
		x1 := real(start)
		y1 := imag(start)
		x2 := real(end)
		...
		y := y1
		for i := 0; i < ncells; i++ {
			row := int((asp.ymax-y)*yscale + .5)
			col := int((x-asp.xmin)*xscale + .5)
			asp.plot.Grid[row*columns+col] = "edgeSP"
			x += stepX
			y += stepY
		}

		// Mark the edge start vertex v.  CSS colors the vertex Black.
		row := int((asp.ymax-y1)*yscale + .5)
		col := int((x1-asp.xmin)*xscale + .5)
		asp.plot.Grid[row*columns+col] = "vertex"

		// Mark the edge end vertex w.  CSS colors the vertex Black.
		row = int((asp.ymax-y2)*yscale + .5)
		col = int((x2-asp.xmin)*xscale + .5)
		asp.plot.Grid[row*columns+col] = "vertex"
	...
	e = asp.edgeTo[asp.target]
	x := real(asp.location[e.w])
	y := imag(asp.location[e.w])
	// Mark the SP end vertex.  CSS colors the vertex Red.
	row := int((asp.ymax-y)*yscale + .5)
	col := int((x-asp.xmin)*xscale + .5)
	asp.plot.Grid[row*columns+col] = "vertexSP2"
	asp.plot.Grid[(row+1)*columns+col] = "vertexSP2"
	asp.plot.Grid[(row-1)*columns+col] = "vertexSP2"
	asp.plot.Grid[row*columns+col+1] = "vertexSP2"
	asp.plot.Grid[row*columns+col-1] = "vertexSP2"

	asp.plot.TargetLocation = fmt.Sprintf("(%.2f, %.2f)", x, y)
	asp.plot.Target = strconv.Itoa(e.w)

	// Mark the SP start vertex.  CSS colors the vertex Blue.
	x = real(asp.location[firstEdge.v])
	y = imag(asp.location[firstEdge.v])
	row = int((asp.ymax-y)*yscale + .5)
	col = int((x-asp.xmin)*xscale + .5)
	asp.plot.Grid[row*columns+col] = "vertexSP1"
	asp.plot.Grid[(row+1)*columns+col] = "vertexSP1"
	asp.plot.Grid[(row-1)*columns+col] = "vertexSP1"
	asp.plot.Grid[row*columns+col+1] = "vertexSP1"
	asp.plot.Grid[row*columns+col-1] = "vertexSP1"

	asp.plot.SourceLocation = fmt.Sprintf("(%.2f, %.2f)", x, y)
	asp.plot.Source = strconv.Itoa(firstEdge.v)

	// Distance of the SP
	asp.plot.DistanceSP = fmt.Sprintf("%.2f", distance)
...
// HTTP handler for /acyclicsp connections
func handleAcyclicSP(w http.ResponseWriter, r *http.Request) {

	// Create the Prim MST instance
	primmst := &PrimMST{}

	// Create the Acyclic SP instance
	acyclicsp := &AcyclicSP{}
	...
	// Assign vertex locations to acyclicsp so it can use x,y coordinates of vertices
	acyclicsp.location = primmst.location
	// Assign graph to acyclicsp so it can use distances between vertices
	acyclicsp.graph = primmst.graph
	// Assign MST to acyclicsp so it can use it to construct adj
	acyclicsp.mst = primmst.mst
	// Assign endpoints to acyclicsp for plotting on the grid
	acyclicsp.Endpoints = primmst.Endpoints

	// Find the Shortest Path
	err = acyclicsp.findSP(r)
	if err != nil {
	...
	// Assign plot to acyclicsp
	acyclicsp.plot = primmst.plot

	// Draw SP into 300 x 300 cell 2px grid
	err = acyclicsp.plotSP()
	if err != nil {
	...
	// Status
	if len(status) > 0 {
		acyclicsp.plot.Status = strings.Join(status, ", ")
	} else {
		acyclicsp.plot.Status = "Enter Source and Target Vertices (0-V-1) for another SP"
	}
...
func main() {
	...
	// Set up http servers with handler for Graph Options and Acyclic SP
	http.HandleFunc(patternAcyclicSP, handleAcyclicSP)
	http.HandleFunc(patternGraphOptions, handleGraphOptions)
	fmt.Printf("Acyclic Shortest Path Server listening on %v.\n", addr)
